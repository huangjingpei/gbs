...
#include "dto/GBSUserInfo.h"

GBSNormalLoginForm::GBSNormalLoginForm(QWidget *parent)
	: QWidget(parent),
...
void GBSNormalLoginForm::onLoginGBS() {
	...
	OBSHttpClient::getInstance()->loginWithCheckVersion(
		email.toUtf8().constData(), password.toUtf8().constData(), 5,
		"多多客", [this](int result) {
			if (result == 0) {
